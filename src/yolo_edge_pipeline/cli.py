"""CLI orchestration for the YOLO edge inference project."""
...
def build_argument_parser() -> argparse.ArgumentParser:
    """Create the CLI parser so it can also be tested independently."""
...
def validate_arguments(arguments: argparse.Namespace, parser: argparse.ArgumentParser) -> None:
    """Validate source-specific arguments with argparse-friendly errors."""
...
def run_image_inference(
    ...
    """Run one-shot inference on a single image."""
...
def log_detection_summary(frame_packet: "FramePacket", detections: tuple) -> None:
    """Log a compact detection summary for observability."""
    LOGGER.info(
...
def sanitize_source_name(source_name: str) -> str:
    """Create a file-system-safe identifier."""
